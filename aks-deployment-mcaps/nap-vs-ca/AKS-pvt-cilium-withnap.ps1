...
# Enable App Routing add-on with internal nginx ingress controller
az aks approuting enable `
    --resource-group $RESOURCE_GROUP `
    --name $AKS_NAME

$INGRESS_SUBNET_NAME = "aks-subnet"   # <-- change to your desired subnet name

az aks approuting update `
    --resource-group $RESOURCE_GROUP `
    --name $AKS_NAME `
    --nginx Internal `
    --nginx-load-balancer-annotations "service.beta.kubernetes.io/azure-load-balancer-internal=true,service.beta.kubernetes.io/azure-load-balancer-internal-subnet=$INGRESS_SUBNET_NAME"
Write-Host "App Routing enabled with Internal nginx ingress controller on subnet: $INGRESS_SUBNET_NAME"              
...
az network public-ip create --name appgwingresspip --resource-group azure-vk-rg --allocation-method Static --sku Standard
az network application-gateway create --name appgwingress --resource-group azure-vk-rg --sku Standard_v2 --public-ip-address appgwingresspip --vnet-name aks-vk-vnet --subnet appgw-subnet --priority 100

# Enable AGIC Addon in AKS
appgwId=$(az network application-gateway show --name appgwingress --resource-group azure-vk-rg -o tsv --query "id") 
az aks enable-addons --name aks-vk-with-cilium --resource-group azure-vk-rg --addon ingress-appgw --appgw-id $appgwId